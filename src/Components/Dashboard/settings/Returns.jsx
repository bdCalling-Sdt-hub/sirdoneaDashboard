...
const Returns = () => {
  ...
  useEffect(() => {
    if (getSettingsData?.data.returns) {
      setContent(getSettingsData.data.returns); // Load the latest policy
    }
  }, [getSettingsData]);
  ...
  const handleOnSave = async () => {
    try {
      ...
      const res =  await updateSettings({ returns: content }).unwrap();
      console.log('res====',res);
     ...
     const res =  await addSettings({ returns: content }).unwrap();
     console.log('res',res);
     if(res.success){
        Swal.fire({
          icon: "success",
